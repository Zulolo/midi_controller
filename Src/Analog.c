...
#define BASE_SENSOR_ADC_AVG_BUF_NUM		4
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	if (hadc->Instance == ADC1){
		HAL_ADC_Stop_DMA(&BASE_SENSOR_ADC_HANDLER);
...
{
	static uint16_t unBuff[2][BASE_SENSOR_ADC_CHN_NUM * BASE_SENSOR_ADC_AVG_BUF_NUM];
	...
	HAL_ADC_Start_DMA(&BASE_SENSOR_ADC_HANDLER, (uint32_t *)(unBuff[unBufSelection]), BASE_SENSOR_ADC_CHN_NUM * BASE_SENSOR_ADC_AVG_BUF_NUM);
	while(1){
